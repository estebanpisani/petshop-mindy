...
function filtrado() {
    ...
    if (searchValue === "" && rangoMin > 0 && rangoMax === 0) {
        ...
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio >= rangoMin);
    }
    // Si sólo hay rango max
    if (searchValue === "" && rangoMin === 0 && rangoMax > 0) {
        // console.log("Entré en max");
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio <= rangoMax);
    }
    // Si hay ambos rangos
    if (searchValue === "" && rangoMin > 0 && rangoMax > rangoMin) {
        // console.log("Entré en min-max");
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio >= rangoMin && articulo.precio <= rangoMax && !articulosFiltrados.includes(articulo));
    }
    // Si sólo hay texto
    if (searchValue !== "" && rangoMin === 0 && rangoMax === 0) {
        // console.log("Entré en solo texto");
        listaMedicamentos.forEach(articulo => {
    ...
    if (searchValue !== "" && rangoMin > 0 && rangoMax === 0) {
        ...
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio >= rangoMin && articulo.nombre.toLowerCase().includes(searchValue.trim().toLowerCase()));
    }
    // Si hay texto y min
    if (searchValue !== "" && rangoMin === 0 && rangoMax > 0) {
        // console.log("Entré en txt y max"); 
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio <= rangoMax && articulo.nombre.toLowerCase().includes(searchValue.trim().toLowerCase()));
    }
    // Si están todos
    if (searchValue !== "" && rangoMin > 0 && rangoMax > rangoMin) {
        // console.log("Entré en todos"); 
        articulosFiltrados = listaMedicamentos.filter(articulo => articulo.precio >= rangoMin && articulo.precio <= rangoMax && articulo.nombre.toLowerCase().includes(searchValue.trim().toLowerCase()));
    }
    //Si no hay ninguno
    if (searchValue === "" && rangoMin === 0 && rangoMax === 0) {
        // console.log("Entré en ninguno.");
        articulosFiltrados = listaMedicamentos;
    }
...
async function getAPIData(url) {
    ...
    listaMedicamentos = filtrarMedicamentos(productosTotal);

    aplicarResetFiltros();
    aplicarFiltroNombre();
    aplicarFiltroPrecio()
    filtrado();
    mostrarCards(listaMedicamentos);
